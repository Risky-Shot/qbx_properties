...
function EnterProperty(playerSource, id, isSpawn)
    local property = MySQL.single.await('SELECT * FROM appartments WHERE id = ?', {id})
...
local function exitProperty(playerSource, isLogout)
    ...
    if not isLogout then
        local enterCoords = json.decode(MySQL.single.await('SELECT coords FROM appartments WHERE id = ?', {enteredProperty[playerSource]}).coords)
...
lib.callback.register('qbx_properties:callback:loadProperties', function()
    local result = MySQL.query.await('SELECT coords FROM appartments GROUP BY coords')
    local properties = {}
    for i = 1, #result do
...
lib.callback.register('qbx_properties:callback:requestProperties', function(_, propertyCoords)
    return MySQL.query.await('SELECT property_name, owner, id, price, rent_interval, keyholders FROM appartments WHERE coords = ?', {json.encode(propertyCoords)})
end)

local function hasAccess(citizenId, propertyId)
    local property = MySQL.single.await('SELECT owner, keyholders FROM appartments WHERE id = ?', {propertyId})
...
RegisterNetEvent('qbx_properties:server:ringProperty', function(data)
    local playerSource = source --[[@as number]]
    local propertyId = data.id
    local property = MySQL.single.await('SELECT owner FROM appartments WHERE id = ?', {propertyId})
...
lib.callback.register('qbx_properties:callback:requestKeyHolders', function(source)
    ...
    local result = MySQL.single.await('SELECT owner, keyholders FROM appartments WHERE id = ?', {propertyId})
...
RegisterNetEvent('qbx_properties:server:letRingerIn', function(visitorCid)
    ...
    local result = MySQL.single.await('SELECT owner, interior FROM appartments WHERE id = ?', {propertyId})
...
RegisterNetEvent('qbx_properties:server:addKeyholder', function(keyholderCid)
    ...
    MySQL.Sync.execute('UPDATE appartments SET keyholders = ? WHERE id = ?', {json.encode(keyholders), propertyId})
...
RegisterNetEvent('qbx_properties:server:logoutProperty', function()
    ...
    local result = MySQL.single.await('SELECT owner, coords FROM appartments WHERE id = ?', {propertyId})
    local player = exports.qbx_core:GetPlayer(playerSource)
    if player.PlayerData.citizenid ~= result.owner then return end
...
AddEventHandler('playerDropped', function ()
    ...
    Wait(50)
    local coords = json.decode(MySQL.single.await('SELECT coords FROM appartments WHERE id = ?', {enteredProperty[playerSource]}).coords)
...
local function startRentThread(propertyId)
    CreateThread(function()
        while true do
            local property = MySQL.single.await('SELECT owner, price, rent_interval, property_name FROM appartments WHERE id = ?', {propertyId})
        ...
        MySQL.update('UPDATE appartments SET owner = ? WHERE id = ?', {nil, propertyId})
    end)
end
...
RegisterNetEvent('qbx_properties:server:rentProperty', function(propertyId)
    ...
    local player = exports.qbx_core:GetPlayer(playerSource)
    local playerCoords = GetEntityCoords(GetPlayerPed(playerSource))
    local property = MySQL.single.await('SELECT owner, price, property_name, coords, rent_interval FROM appartments WHERE id = ?', {propertyId})
    local propertyCoords = json.decode(property.coords)
    ...
    MySQL.update('UPDATE appartments SET owner = ? WHERE id = ?', {player.PlayerData.citizenid, propertyId})
    startRentThread()
end)
...
RegisterNetEvent('qbx_properties:server:buyProperty', function(propertyId)
    ...
    local player = exports.qbx_core:GetPlayer(playerSource)
    local playerCoords = GetEntityCoords(GetPlayerPed(playerSource))
    local property = MySQL.single.await('SELECT owner, price, property_name, coords FROM appartments WHERE id = ?', {propertyId})
    local propertyCoords = json.decode(property.coords)
...
CreateThread(function()
    local properties = MySQL.query.await('SELECT id FROM appartments WHERE owner IS NOT NULL AND rent_interval IS NOT NULL')
...
RegisterNetEvent('qbx_properties:server:stopRenting', function()
    local player = exports.qbx_core:GetPlayer(source)
    local propertyId = enteredProperty[source]
    local property = MySQL.single.await('SELECT owner, property_name FROM appartments WHERE id = ?', {propertyId})
    ...
    MySQL.update.await('UPDATE appartments SET owner = ?, keyholders = JSON_OBJECT() WHERE id = ?', {nil, propertyId})
